...
#define AMP_ASSERT(x,...)  if(!(x)) \
                                { \
                                    AMP_LOG_Print(AMP_LOG_ADDR_WIMAX_QA, AMP_LOG_LEVEL_DEBUG2, __VA_ARGS__); \
...
// /tmp, in windows, temporarily I use inet socket.
// FIXME, we need to determine an interface for windows to use on SME 
#ifdef WIN32
#define AMP_SME_LOCAL_WIMAX_SOCKET_PATH   "8000"
#define AMP_IOCTL_LOCAL_WIMAX_SOCKET_PATH   "9000"
#define INLINE _inline
#elif defined(__KERNEL__) && !defined(HOST_EMULATION)
    // nothing need to be done now
#define INLINE inline
#else
#ifdef CONFIG_LTE
#define AMP_SME_LOCAL_WIMAX_SOCKET_PATH   "/tmp/appltesock"
#define AMP_IOCTL_LOCAL_WIMAX_SOCKET_PATH "/tmp/lteioctlso"
#else
#define AMP_SME_LOCAL_WIMAX_SOCKET_PATH   "/tmp/smewimaxsock"
#endif
#define INLINE inline
#endif
