...
#define OMAP2_SRAM_PUB_VA   (OMAP2_SRAM_VA + 0xF800)
#else
...
#ifdef CONFIG_ARCH_OMAPW3G /* For wrigley SRAM base is same as OMAP24XX */
#define OMAPW3G_VA_REQINFOPERM0 IO_ADDRESS(0x46012848)
#define OMAPW3G_VA_READPERM0    IO_ADDRESS(0x46012850)
#define OMAPW3G_VA_WRITEPERM0   IO_ADDRESS(0x46012858)
#endif
...
static int is_sram_locked(void)
{
	...
	if (type == GP_DEVICE) {
		...
		if (cpu_is_omapw3g()) {
			__raw_writel(0xFFFFFFFF, OMAPW3G_VA_REQINFOPERM0);
			__raw_writel(0xFFFF, OMAPW3G_VA_READPERM0);
			__raw_writel(0xFFFF, OMAPW3G_VA_WRITEPERM0);
		}
		return 0;
	} else
	...
	if (cpu_class_is_omap2()) {
		if (is_sram_locked()) {
			...
			if (cpu_is_omap34xx()) {
				...
				else if (cpu_is_omap243x() || cpu_is_omapw3g())/* Wrigley = 64k */
					omap_sram_size = 0x10000; /* 64K */
			}
		}
	} else {
...
#if defined(CONFIG_ARCH_OMAP3) || defined(CONFIG_ARCH_OMAPW3G)
...
int __init omap_sram_init(void)
{
	...
	if (!(cpu_class_is_omap2()))
		...
		omap243x_sram_init();
	else if (cpu_is_omap34xx() || cpu_is_omapw3g())
		omap3_sram_init();
