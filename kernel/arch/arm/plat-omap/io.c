...
void __iomem *omap_ioremap(unsigned long p, size_t size, unsigned int type)
{
...
#endif
#if (defined( CONFIG_ARCH_OMAP2) && !defined(CONFIG_ARCH_OMAPW3G))
	if (cpu_is_omap24xx()) {
    ...
    if(BETWEEN(p, L3_W3G_PHYS, L3_W3G_SIZE))
        return XLATE(p, L3_W3G_PHYS, L3_W3G_VIRT);
    if(BETWEEN(p, L3_W3G_INC_PHYS, L3_W3G_INC_SIZE))
        return XLATE(p, L3_W3G_INC_PHYS, L3_W3G_INC_VIRT);
    if(BETWEEN(p,W3G_IPC_SHARED_MEMORY_PHYS, W3G_IPC_SHARED_MEMORY_SIZE))
        return XLATE(p, W3G_IPC_SHARED_MEMORY_PHYS, W3G_IPC_SHARED_MEMORY_VIRT);	
#ifdef CONFIG_MACH_W3G_LTE_DATACARD   
    if(BETWEEN(p,W3G_4GHIF_IPC_SHARED_MEMORY_PHYS,W3G_4GHIF_IPC_SHARED_MEMORY_SIZE))
        return XLATE(p,W3G_4GHIF_IPC_SHARED_MEMORY_PHYS,W3G_4GHIF_IPC_SHARED_MEMORY_VIRT);   
#endif 
    if(BETWEEN(p, L4_W3G_APPS_PHYS, L4_W3G_APPS_SIZE))
        return XLATE(p, L4_W3G_APPS_PHYS, L4_W3G_APPS_VIRT);
    if(BETWEEN(p, L3_MDMDSP_PHYS, L3_MDMDSP_SIZE))
        return XLATE(p, L3_MDMDSP_PHYS, L3_MDMDSP_VIRT);
    if(BETWEEN(p, L3_L4_PER_PHYS, L3_L4_PER_SIZE))
        return XLATE(p, L3_L4_PER_PHYS, L3_L4_PER_VIRT);
    if(BETWEEN(p, L4_3G_BASE_PHYS, L4_3G_BASE_SIZE))
        return XLATE(p, L4_3G_BASE_PHYS, L4_3G_BASE_VIRT);
    if(BETWEEN(p, L4_EMU_BASE_PHYS, L4_EMU_BASE_SIZE))
        return XLATE(p, L4_EMU_BASE_PHYS, L4_EMU_BASE_VIRT);
