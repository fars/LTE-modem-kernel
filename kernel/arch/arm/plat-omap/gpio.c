...
/*
 * omapW3G specific GPIO registers
 */
#define OMAPW3G_GPIO1_BASE		IO_ADDRESS(0x5C1D0000)
#define OMAPW3G_GPIO2_BASE		IO_ADDRESS(0x5C1B2000)
#define OMAPW3G_GPIO3_BASE		IO_ADDRESS(0x5C1B4000)
...
static struct gpio_bank gpio_bank_W3G[3] = {
	{ OMAPW3G_GPIO1_BASE, INT_24XX_GPIO_BANK1, IH_GPIO_BASE,	METHOD_GPIO_24XX },
	{ OMAPW3G_GPIO2_BASE, INT_24XX_GPIO_BANK2, IH_GPIO_BASE + 32,	METHOD_GPIO_24XX },
	{ OMAPW3G_GPIO3_BASE, INT_24XX_GPIO_BANK3, IH_GPIO_BASE + 64,	METHOD_GPIO_24XX },
};
#endif
...
static struct clk *gpio_iclks[3];
#endif
...
static int __init _omap_gpio_init(void)
{
	...
	if (cpu_is_omap243x()) {
		...
		gpio_bank_count = 3;
		gpio_bank = gpio_bank_W3G;
		rev = __raw_readl(gpio_bank[0].base + OMAP24XX_GPIO_REVISION);
		printk(KERN_INFO "OMAPW3G GPIO hardware version %d.%d\n",
			(rev >> 4) & 0x0f, rev & 0x0f);
	}
#endif
#ifdef CONFIG_ARCH_OMAP34XX
	...
	for (i = 0; i < gpio_bank_count; i++) {
		...
		if (bank->method == METHOD_GPIO_24XX) {
			...
			__raw_writel(0x00000000, bank->base + OMAP24XX_GPIO_DEBOUNCE_EN);
                        ...
                        if (cpu_is_omapw3g()) {
				/* Wirte the GPIO_OE register of GPIO module 1,2,3 to enable INPUT or OUTPUT and
				 Wirte the GPIO_DATAOUT register of GPIO module 1,2,3 to drive the GPIO HIGH or LOW */
				if (i == 0) {
					__raw_writel(0xFFFFFFFF, bank->base + OMAP24XX_GPIO_OE);
					__raw_writel(0x00000000, bank->base + OMAP24XX_GPIO_SETDATAOUT);
				}
				else if (i == 1) {
					__raw_writel(0x7FFFFC07, bank->base + OMAP24XX_GPIO_OE);
					__raw_writel(0x800003F8, bank->base + OMAP24XX_GPIO_SETDATAOUT);
				}
				else if(i == 2) {
#ifdef CONFIG_MACH_W3G_LTE_DATACARD
					__raw_writel(0xFDF75EFF, bank->base + OMAP24XX_GPIO_OE);
					__raw_writel(0x0202A100, bank->base + OMAP24XX_GPIO_SETDATAOUT);
#else
					__raw_writel(0xFDFFDFFF, bank->base + OMAP24XX_GPIO_OE);
					__raw_writel(0x02002000, bank->base + OMAP24XX_GPIO_SETDATAOUT);
#endif
				}
			}
	...
	if (cpu_is_omap24xx()) {
		if (cpu_is_omapw3g()) {
			/* Enable autoidle for GPIO1 */
			omap_writel(omap_readl(0x5C184C30) | 0x08, 0x5C184C30);
			/* Enable autoidle for GPIO2 & GPIO3 */
			omap_writel(omap_readl(0x5C185530) | 0x03, 0x5C185530);
		}
		else {
...
static int omap_gpio_suspend(struct sys_device *dev, pm_message_t mesg)
{
	...
	/* W3G MPU has no ownership of GPIO wakeup configuration. */
	if (cpu_is_omapw3g())
		return 0;
...
void omap2_gpio_prepare_for_idle(int power_state)
{
	...
	/* GPIO 1 is in WKUP domain, always on, not part of idle */
	if (cpu_is_omap34xx() || cpu_is_omapw3g())
		min = 1;
