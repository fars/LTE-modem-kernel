...
extern void omap_prcm_arch_pm_reset(char mode); /*For W3G declared here*/
#endif

#if defined(CONFIG_ARCH_OMAPW3G)
#define GPIO3_FCK                 "gpio3_dbck"
#define CONTROL_PADCONF_GPIO_87   IO_ADDRESS(0x5C182238)
#define GPIO_SIGNAL_87            87
...
void omap_prcm_arch_reset(char mode)
{
	...
	if (cpu_is_omapw3g())
	{
		...
		gpio3_clk = clk_get(NULL, GPIO3_FCK);
		clk_enable(gpio3_clk);

		/* Get GPIO87 (sys_nreswarm), configure padconf for gpio87 output, and drive
		 * the pad low for 10ms to force a CPCAP restart.  If this doesn't generate a
		 * proper restart, the global warm reset will be taken as fallback.
		 */
		status = gpio_request(GPIO_SIGNAL_87, NULL);
		if (status == 0)
		{
				write_reg_bits((volatile unsigned int *)CONTROL_PADCONF_GPIO_87, (0xFFFF0000), (0x00020000));
				gpio_direction_output(GPIO_SIGNAL_87, 1);
				msleep(10);
				gpio_set_value(GPIO_SIGNAL_87,0);
				msleep(10);
		}
#endif
		prcm_offs = OMAPW3G_GR_MOD;
  }
