...
int clkdm_register(struct clockdomain *clkdm)
{
	...
	if (!pwrdm) {
		pr_err("clockdomain: clkdm_register %s: powerdomain %s "
	...
	pr_err("clockdomain: registered %s\n", clkdm->name);
	ret = 0;
...
int omap2_clkdm_sleep(struct clockdomain *clkdm)
{
	...
	if (cpu_is_omap24xx() && !cpu_is_omapw3g()) {
...
void omap2_clkdm_allow_idle(struct clockdomain *clkdm)
{
	...
	if (cpu_is_omapw3g())
		v = OMAPW3G_CLKSTCTRL_ENABLE_AUTO;
...
void omap2_clkdm_deny_idle(struct clockdomain *clkdm)
{
	...
	if (cpu_is_omapw3g())
		v = OMAPW3G_CLKSTCTRL_DISABLE_AUTO;
	else if (cpu_is_omap24xx())
...
int omap2_clkdm_clk_enable(struct clockdomain *clkdm, struct clk *clk)
{
	...
	if ((cpu_is_omap34xx() && v == OMAP34XX_CLKSTCTRL_ENABLE_AUTO) ||
       ((cpu_is_omap24xx() && !cpu_is_omapw3g()) &&
	       v == OMAP24XX_CLKSTCTRL_ENABLE_AUTO) ||
       (cpu_is_omapw3g() && v == OMAPW3G_CLKSTCTRL_ENABLE_AUTO))
		_clkdm_add_autodeps(clkdm);
	else
