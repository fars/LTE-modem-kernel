/*
 * linux/arch/arm/mach-omap2/board-hpm1200l-hsmm.c
 *
...
#define GPIO_SIGNAL_MMC_DET 30 /* W3G */
...
extern int hpm1200l_wifi_status(int irq);
#ifdef CONFIG_MMC_EMBEDDED_SDIO
extern int hpm1200l_wifi_status_register(void (*callback)(int card_present,
						void *dev_id), void *dev_id);
#endif

static int hsmmc2_card_detect(int irq)
{
	return hpm1200l_wifi_status(irq);
}
#endif
...
static int hsmmc_late_init(struct device *dev)
{
	...
	hsmmc_regulator = regulator_get(NULL, "vsdio");
	if (IS_ERR(hsmmc_regulator)) {
		dev_dbg(dev, "vsdio regulator missing\n");
				...
				int vdd)
{
	...
	if (power_on) {
		...
		/* power ON */
		reg = omap_ctrl_readl(OMAPW3G_CONTROL_PBIAS_LITE);
		reg &= ~OMAP2_MMCPBIASSPEEDCTRL0;
		reg &= ~OMAP2_MMCPBIASLITEPWRDNZ0;
		reg &= ~OMAP2_MMCPBIASLITEPWRDNZIO;

		omap_ctrl_writel(reg, OMAPW3G_CONTROL_PBIAS_LITE);
		...
		reg = omap_ctrl_readl(OMAPW3G_CONTROL_PBIAS_LITE);
		reg |= (OMAP2_MMCPBIASLITEPWRDNZ0 | OMAP2_MMCPBIASLITEVMODE0);
...
static struct embedded_sdio_data hpm1200l_wifi_emb_data = {
	.cis    = {
		.vendor         = 0x104c,
		.device         = 0x9066,
		.blksize        = 512,
...
#ifdef CONFIG_MMC_EMBEDDED_SDIO
		.embedded_sdio		= &hpm1200l_wifi_emb_data,
		.register_status_notify	= &hpm1200l_wifi_status_register,
#endif
	},
};
#endif
...
void __init hpm1200l_hsmmc_init(void)
{
	...
	set_irq_type(mmc1_data.slots[0].card_detect_irq, IRQ_TYPE_EDGE_RISING);
	omap2_init_mmc(hsmmc_data, OMAPW3G_NR_MMC);
}
