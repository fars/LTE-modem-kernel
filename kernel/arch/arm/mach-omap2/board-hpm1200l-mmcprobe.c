/*
 * arch/arm/mach-omap2/board-hpm1200l-mmcprobe.c
 *
...
void hpm1200l_mmcprobe_strobe(int ch, int rpt)
{
	int i;
...
void hpm1200l_mmcprobe_set(int ch, int lvl)
{
	gpio_direction_output(CH_BASE_GPIO + ch, lvl);
}

void __init hpm1200l_mmcprobe_init(void)
{
	int i;

	printk(KERN_INFO "hpm1200l_mmcprobe: MMC subsystem ganked for debug\n");
	...
	for (i = 0; i < 5; i++)
		hpm1200l_mmcprobe_strobe(i, 1 + i);
}
