...
static inline void omap_uart_enable_rtspullup(struct omap_uart_state *uart)
{
	...
	omap_ctrl_writew(0x18 | 0x2, uart->rts_padconf);
	uart->rts_override = 1;
}
...
static void omap_uart_rtspad_init(struct omap_uart_state *uart)
{
	if (cpu_is_omapw3g()) {
		switch (uart->num) {
		case 0:
			uart->rts_padconf = 0; /* 0x1C4 */
			break;
		case 1:
			uart->rts_padconf = 0x182;
			break;
		case 2:
			uart->rts_padconf = 0x18C;
			break;
		default:
			uart->rts_padconf = 0;
			break;
...
static void omap_uart_idle_init(struct omap_uart_state *uart)
{
	...
	if (cpu_is_omapw3g()) {
		...
		/* WK EN in CORE domain is not configured by MPU */
		uart->wk_en = 0;
		uart->wk_st = OMAPW3G_PRM_REGADDR(OMAPW3G_CORE_MOD, PM_WKST);
		switch (uart->num) {
		case 0:
			wk_mask = OMAPW3G_ST_UART1;
			padconf = 0;
			break;
		case 1:
			wk_mask = OMAPW3G_ST_UART2;
			padconf = 0x184;
			break;
		case 2:
			wk_mask = OMAPW3G_ST_UART3;
			padconf = 0x18e;
			break;
		}
...
/*			padconf = 0x180; */
	...
	if ((cpu_is_omap34xx() || cpu_is_omapw3g()) && uart->padconf) {
		u16 v;
...
DEFINE_SIMPLE_ATTRIBUTE(fifo_idleblk_fops,
			      fifo_idleblk_get,
			      fifo_idleblk_set,
			      "%llu\n");
			     ...
			     unsigned int wake_strobe_enable_mask)
{
	...
	debugfs_create_file("fifo_idle_block_count",
			    0644, NULL, NULL,
			    &fifo_idleblk_fops);
	/*
