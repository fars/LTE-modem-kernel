...
#else
	n = alloc_skb(((skb->end - skb->head + skb->data_len) + 2), gfp_mask);
#endif
	if (!n)
		return NULL;
...
#else
	n = alloc_skb(((skb->end - skb->head) + 2), gfp_mask);
#endif
	if (!n)
		goto out;
				...
				gfp_t gfp_mask)
{
	 ...
	 *	Allocate the copy buffer
	 */
  struct sk_buff *n = alloc_skb((newheadroom + skb->len + newtailroom + 2),
				      gfp_mask);
...
struct sk_buff *skb_segment(struct sk_buff *skb, int features)
{
	...
	do {
		...
		if (!hsize && i >= nfrags) {
		  ...
		  nskb = alloc_skb((hsize + doffset + headroom + 2),
					 GFP_ATOMIC);
