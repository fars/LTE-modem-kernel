 ...
 *! Revision History:
 *! ================
 *! 16 Jul 2009 Motorola Add new function to read IRQ status
...
/* Gets the status of the specified IRQ. */
HW_STATUS HW_MBOX_EventStatus(const u32 baseAddress,
			      const HW_MBOX_Id_t mailBoxId,
			      const HW_MBOX_UserId_t userId,
			      const u32 event,
...
{	
	...
	CHECK_INPUT_PARAM(baseAddress, 0, RET_BAD_NULL_PARAM,
			  RES_MBOX_BASE + RES_INVALID_INPUT_PARAM);
	CHECK_INPUT_PARAM(pEventStatus, NULL, RET_BAD_NULL_PARAM,
			  RES_MBOX_BASE + RES_INVALID_INPUT_PARAM);
	...
	*pEventStatus = MLBMAILBOX_IRQSTATUS___0_3ReadRegister32(baseAddress,
								 (u32)userId);

	*pEventStatus = ((*pEventStatus) & (((u32)event) <<
					 (((u32)(mailBoxId)) *
					  HW_MBOX_ID_WIDTH))) ? 1: 0;
	return RET_OK;
}
