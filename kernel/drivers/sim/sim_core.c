...
static struct clk *gpio3_iclk;
static struct clk *gpio3_fclk;
...
#else
#define DMA_SYSCONFIG IO_ADDRESS(0x4700A02C)
#define OMAPW3G_DMA_USIM_RX 56
/* Could not find CM interface for Auto idle */
#define CM_AUTOIDLE_WK IO_ADDRESS(OMAPW3G_CM_BASE + 0x0A30)
#define PBIAS_CONTROL_LITE IO_ADDRESS(OMAPW3G_SCM_BASE + 0x05E0)
#define CONTROL_PADCONF_TEST_TME IO_ADDRESS(OMAPW3G_SCM_BASE + 0x0270)
#define CONTROL_PADCONF_KBD_C_7  IO_ADDRESS(OMAPW3G_SCM_BASE + 0x0224)
#define CONTROL_PADCONF_USIM_CLK IO_ADDRESS(OMAPW3G_SCM_BASE + 0x0228)
#define CONTROL_PADCONF_USIM_RST IO_ADDRESS(OMAPW3G_SCM_BASE + 0x022C)
...
#else
#define PBIASVMODE1        0x0008
#define PBIASPWRDNZ1       0x0010
#define PBIASSPEEDCNTL1    0x0020
#define PBIASLITEPWRDNZIO  0x0200
/* PBIASVMODEERROR1  is connected to GPIO_94 */
#endif
...
#else
#define SIM_ICLK_AUTOIDLE  0x0400
#endif

#ifdef CONFIG_ARCH_OMAPW3G
/* Mask for USIM clock pad config register */
#define PADCONF_USIM_CLK_MUX_SAFE_MODE   0x0003
#define PADCONF_USIM_CLK_PULLUDENABLE0   0x0008
#define PADCONF_USIM_CLK_PULLTYPESELECT0 0x0010
#define PADCONF_USIM_CLK_INPUT_ENABLE0   0x0100
#define PADCONF_USIM_CLK_STANDBYENABLE0  0x0200

/* Mask for USIM_PBIAS */
#define CONTROL_PADCONF_TEST_TME_MASK    0x00180000
/* Mask for USIM_IO */
#define CONTROL_PADCONF_KBD_C_7_MASK     0x00180000
/* Mask for USIM_CLK and USIM_PWRCTL */
#define CONTROL_PADCONF_USIM_CLK_MASK    0x00100018
/* Mask for USIM_RST and USIM_CD */
#define CONTROL_PADCONF_USIM_RST_MASK    0x01100018
/* GPIO for USIM_PBIAS_ERROR */
#define USIM_PBIAS_ERROR_GPIO_VAL_94     94
...
#define USIM32K_FCK "usim32k_fck"
#define USIM_ICK "usim_ick"
#define GPIO3_FCLK "gpio3_dbck"
#define GPIO3_ICLK "gpio3_ick"
#ifndef CONFIG_ARCH_OMAPW3G
# define OMAP_96M_FCK "cm_96m_fck"
# define OMAP_120M_FCK "dpll5_m2_ck"
#else
# define OMAP_96M_FCK "dpll4_m2_ck"
# define OMAP_60M_FCK "dpll4_m3_ck"
#endif
/******************************************************************************
 * Local Macros
 *****************************************************************************/
...
static UINT8 sim_buffer[SIM_MODULE_MAX_DATA];
static UINT16 rx_current_offset=0;
		     ...
		     unsigned int cmd, unsigned long arg)
{
	...
	static unsigned int num_rx_char=0;
	...
	switch (cmd) {
#ifdef CONFIG_ARCH_OMAPW3G
	case SIM_IOCTL_CONFIG_CLOCK_PAD:
		status =
		    copy_from_user(args_kernel, (UINT32 *) arg,
				   CMD_PARAMETER_SIZE_2);
		if (args_kernel[0] < NUM_SIM_MODULES) {
			if(args_kernel[1]==SIM_MODULE_CLOCK_CONFIG_NORMAL_MODE) {
				/* During activation  configuring as normal mode */
				write_reg_bits((volatile UINT32 *)CONTROL_PADCONF_USIM_CLK,
					       (PADCONF_USIM_CLK_STANDBYENABLE0 | PADCONF_USIM_CLK_INPUT_ENABLE0 |
					       PADCONF_USIM_CLK_MUX_SAFE_MODE  | PADCONF_USIM_CLK_PULLUDENABLE0 ),
					       (PADCONF_USIM_CLK_STANDBYENABLE0 | PADCONF_USIM_CLK_PULLUDENABLE0 ));
			} else if (args_kernel[1]==SIM_MODULE_CLOCK_CONFIG_SAFE_MODE) {
				/* During Deactivation configuring in safe mode */
				write_reg_bits((volatile UINT32 *)CONTROL_PADCONF_USIM_CLK,
					       (PADCONF_USIM_CLK_INPUT_ENABLE0 | PADCONF_USIM_CLK_PULLTYPESELECT0 |
					        PADCONF_USIM_CLK_PULLUDENABLE0 | PADCONF_USIM_CLK_MUX_SAFE_MODE ),
					       (PADCONF_USIM_CLK_INPUT_ENABLE0 | PADCONF_USIM_CLK_PULLUDENABLE0 |
					        PADCONF_USIM_CLK_MUX_SAFE_MODE ));
			}
		}
		break;
#endif
	case SIM_IOCTL_CONFIGURE_GPIO:
		...
		clk_enable(usim_fck);
		clk_enable(usim32k_fck);

		/* sleep some time waiting for clock to stabilize */
		msleep(1);
	...
	case SIM_IOCTL_READ_DATA_BUFFER:
		...
		if (args_kernel[0] < NUM_SIM_MODULES) {
			...
			if (copy_to_user
			    ((UINT8 *) args_kernel[1],
			     (&sim_buffer[args_kernel[2]]), args_kernel[3])) {
				tracemsg
	...
	case SIM_IOCTL_UPDATE_DATA_BUFFER:
		...
		if (args_kernel[0] < NUM_SIM_MODULES) {
			for(tx_offset=0; tx_offset<args_kernel[2]; tx_offset++)
			{
				sim_module_card_data[args_kernel[0]].buffer[tx_offset] =
				    *((UINT8 *) args_kernel[1] + tx_offset);
			}
		} else {
			tracemsg
			    ("Warning: Invalid reader ID in SIM driver request.\n");
	...
	case SIM_IOCTL_INTERRUPT_MODE:
		...
		if ((UINT8) args_kernel[0] == SIM_MODULE_1) {
			...
			case SIM_MODULE_RX_MODE:
				...
				error =
				    omap_request_dma(OMAPW3G_DMA_USIM_RX,
						     SIM_DEV_NAME,
		...
		if (args_kernel[0] < NUM_SIM_MODULES) {
			...
			if (sim_module_dma_channel != 0) {
				...
				if (omap_get_dma_dst_pos
					...
					for (rx_index=rx_current_offset; rx_index < args_kernel[1];
					     rx_index++)
					{
						value = sim_module_card_data[args_kernel[0]].
						    buffer[rx_current_offset];
						if ((value & 0x100) != 0)
						{
							sim_buffer[num_rx_char] =(UINT8)value;
							rx_current_offset++;
							num_rx_char++;
						}
						else
...
static int sim_open(struct inode *inode, struct file *file)
{
	...
	if (status == 0) {
		...
		sim_module_card_data[SIM_MODULE_1].buffer =
		    (UINT16 *) dma_alloc_coherent(NULL, (SIM_MODULE_MAX_DATA*2),
						 &(sim_module_card_data
						   [SIM_MODULE_1].
						   dma_buffer), 0);
...
static void sim_module_set_voltage_level(SIM_MODULE_VOLTAGE_LEVEL level)
{
	...
	if (regulator_enabled_flag) {
                /* vsimcard and then vsim regulators should be disabled */
	...
	write_reg_bits((volatile UINT32 *) PBIAS_CONTROL_LITE,
		       PBIASPWRDNZ1, 0);
#else
        write_reg_bits((volatile UINT32 *)PBIAS_CONTROL_LITE, PBIASPWRDNZ1 | PBIASLITEPWRDNZIO, 0);
#endif
	...
	if (level == SIM_MODULE_VOLTAGE_HIGH) {
		...
		if (!(regulator_enabled_flag)) {
			...
			/* we want to do these pad settings before setting vsimcard */
                        /* configure the pad for 3V operation */
                        write_reg_bits((volatile UINT32 *) PBIAS_CONTROL_LITE,
...
#else
                        /* stabilize the Vdds and the IO pads */
                        write_reg_bits((volatile UINT32 *)PBIAS_CONTROL_LITE,
                                        PBIASPWRDNZ1 | PBIASLITEPWRDNZIO,
                                        PBIASPWRDNZ1 | PBIASLITEPWRDNZIO);
#endif
		...
		if (!(regulator_enabled_flag)) {
			...
			/* we want to do these pad settings before enabling vsimcard */
	...
	if ((level == SIM_MODULE_VOLTAGE_HIGH)
		...
		clk_enable(gpio3_fclk);
		/* the pad bias level doesn't equal the VSIM level ... */
		if (
#ifndef CONFIG_ARCH_OMAPW3G
(read_reg((volatile UINT32 *) PBIAS_CONTROL_LITE) &
			PBIASVMODEERROR1)
#else
               /* Read the USIM_PBIAS_ERROR from GPIO_94 */
               gpio_get_value(USIM_PBIAS_ERROR_GPIO_VAL_94)
#endif
                                  != 0)
 {
			/* block the current thread, but yeild the processor
			   every iteration of the loop */
			printk("USIM_PBIAS_ERROR_GPIO_VAL_94\n");
			msleep(1);
		}
		clk_disable(gpio3_fclk);
				      ...
				      SIM_MODULE_CLOCK_RATE rate)
{
 ...
 /* set the source of the SIM FCLK to
                    3430 --> 120 MHz clock divided by 16
                    W3G  -->  60 MHz clock divided by 8
                    to obtain a 3.75 MHz SIM clock */
                clk_set_parent (usim_fck,
#ifndef CONFIG_ARCH_OMAPW3G
                                          omap_120m_fck
#else
                                          omap_60m_fck
...
int __init sim_init(void)
{
	...
	usim32k_fck = clk_get(&sim_device.dev, USIM32K_FCK);
	if (IS_ERR(usim32k_fck)) {
		tracemsg("sim_init: Error getting USIM32K FCLK.\n");
		ret = PTR_ERR(usim32k_fck);
	}
	...
	gpio3_iclk = clk_get(&sim_device.dev, GPIO3_ICLK);
	if (IS_ERR(gpio3_iclk)) {
		tracemsg("sim_init: Error getting GPIO3 ICLK.\n");
		ret = PTR_ERR(gpio3_iclk);
	}

	gpio3_fclk = clk_get(&sim_device.dev, GPIO3_FCLK);
	if (IS_ERR(gpio3_fclk)) {
		tracemsg("sim_init: Error getting GPIO3 FCLK.\n");
		ret = PTR_ERR(gpio3_fclk);
	}
...
#else
    omap_60m_fck = clk_get(&sim_device.dev, OMAP_60M_FCK);
    if (IS_ERR(omap_60m_fck))
    {
        tracemsg("sim_init: Error getting 60M FCLK.\n");
        ret = PTR_ERR(omap_60m_fck);
    }

    /* Initialize USIM_CD and USIM_RST pad */
    write_reg_bits((volatile UINT32 *)CONTROL_PADCONF_USIM_RST, CONTROL_PADCONF_USIM_RST_MASK, CONTROL_PADCONF_USIM_RST_MASK);

    /* Initialize USIM_IO pad */
    write_reg_bits((volatile UINT32 *)CONTROL_PADCONF_KBD_C_7, CONTROL_PADCONF_KBD_C_7_MASK,CONTROL_PADCONF_KBD_C_7_MASK);

    /*Initialize USIM_CLK and USIM_PWRCTL pad */
    write_reg_bits((volatile UINT32 *)CONTROL_PADCONF_USIM_CLK, CONTROL_PADCONF_USIM_CLK_MASK,CONTROL_PADCONF_USIM_CLK_MASK);

    /* Initialize usim_pbias pad to read PBIASVMODEERROR1 */
    write_reg_bits((volatile UINT32 *)CONTROL_PADCONF_TEST_TME, CONTROL_PADCONF_TEST_TME_MASK, CONTROL_PADCONF_TEST_TME_MASK);
