...
static void mtp_function_suspend(struct usb_function *f)
{
	struct usb_composite_dev *cdev = f->config->cdev;
	DBG(cdev, "mtp suspended\n");

	/* Revisit: MTP interface suspend handling */
}

static void mtp_function_resume(struct usb_function *f)
{
	struct usb_composite_dev *cdev = f->config->cdev;
	DBG(cdev, "mtp resumed\n");

	/* Revisit: MTP interface resume handling */
}
	...
	struct usb_configuration *c)
{
	...
	g_usb_mtp_context.function.suspend = mtp_function_suspend;
	g_usb_mtp_context.function.resume = mtp_function_resume;
