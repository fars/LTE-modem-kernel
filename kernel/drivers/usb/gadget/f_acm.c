...
static struct usb_string acm_string_defs[] = {
	...
	[1].s = "Motorola Data Interface",
#else
	[0].s = "Motorola AT Serial Port Interface",
	[1].s = "CDC ACM Data",
#ifdef CONFIG_USB_GADGET_DUAL_ACM
	[2].s = "Motorola GPS Serial Port Interface",
	[3].s = "CDC ACM Data",
#endif
#endif
	{  /* ZEROES END LIST */ },
...
static void acm_suspend(struct usb_function *f)
{
	struct f_acm *acm = func_to_acm(f);
	struct usb_composite_dev *cdev = f->config->cdev;
	DBG(cdev, "acm ttyGS%d suspended\n", acm->port_num);
}
...
acm_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	/* usb_ram_log: acm_bind */
	usb_ram_log(0x00280001, acm->port_num);
	/* usb_ram_log: acm bind gadget dual speed */
	usb_ram_log(0x00280002, gadget_is_dualspeed(c->cdev->gadget));
#endif
	return 0;
...
int __init acm_bind_config(struct usb_configuration *c, u8 port_num)
{
	...
	if (acm_string_defs[no_acm_count].id == 0) {
		status = usb_string_id(c->cdev);
		if (status < 0)
			return status;
		acm_string_defs[no_acm_count].id = status;
		...
		if (status < 0)
			return status;
		acm_string_defs[no_acm_count+1].id = status;
	...
	struct usb_configuration *c)
{
	...
	for (loop_count = 0; loop_count < NUM_ACM_PORTS; loop_count++) {
		ret = acm_bind_config(c, loop_count);
		if (ret)
			return ret;
	}
}
