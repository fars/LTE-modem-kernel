...
static void adb_suspend(struct usb_function *f)
{
	struct adb_dev  *dev = func_to_dev(f);
	DBG(dev->cdev, "adb suspended\n");
}

static void adb_resume(struct usb_function *f)
{
	struct adb_dev  *dev = func_to_dev(f);
	DBG(dev->cdev, "adb resumed\n");
}
...
adb_function_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	f->descriptors = usb_copy_descriptors(fs_adb_descs);
	if (!f->descriptors)
		goto fail;
	...
	f->hs_descriptors = usb_copy_descriptors(hs_adb_descs);
	if (!f->hs_descriptors)
		goto fail;
	...
	struct usb_configuration *c)
{
	...
	dev->function.suspend = adb_suspend;
	dev->function.resume = adb_resume;
