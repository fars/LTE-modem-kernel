...
#if defined(CONFIG_BATTERY_CPCAP)
...
static int bq24113a_set_power(struct otg_transceiver *x, unsigned int mA)
{
        struct cpcap_usb *cpcap;
        ...
        cpcap = dev_get_drvdata(x->dev);
        bq24113a_charger_set_usb_prop_curr(cpcap->cpcap, mA);
...
#else if defined(CONFIG_CHARGER_BQ24113A)
	cpcap->otg.set_power            = bq24113a_set_power;
#endif
        cpcap->asleep                   = 1;
