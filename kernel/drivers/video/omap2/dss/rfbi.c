...
#define RFBI_SYSCONFIG_BUSY 0x00000100
#define DMA_TRANSFER_FREQUENCY  (32 * 1000 * 1000)
...
void omap_rfbi_write_command(const void *buf, u32 len)
{
	...
	case OMAP_DSS_RFBI_PARALLELMODE_16:
	{
		...
		for (; len; len -= 2) {
			...
			while (rfbi_read_reg(RFBI_SYSCONFIG) &
			    RFBI_SYSCONFIG_BUSY) ;
		}
		break;
	}
		...
		u16 w, u16 h)
{
	...
	rfbi_write_reg(RFBI_PIXEL_CNT, w * h);
	...
	if (rfbi.datatype == OMAP_DSS_RFBI_DATATYPE_16 &&
	   ...
	   rfbi.parallelmode == OMAP_DSS_RFBI_PARALLELMODE_16) {
		...
		for (jj = 0 ; jj < h; jj++) {
			for (ii = 0; ii < w; ii++) {
				rfbi_write_reg(RFBI_DATA, __raw_readw(pd));
				++pd;
			}
			pd += horiz_offset;
		}
...
int rfbi_init(void)
{
	...
	/* NOTE: L4 clock frequency is stored in KHz, so timing */
	/*       parameters like CSon/off, WEon/off and others  */
	/*       should be in "ps" instead of "ns"              */
			...
			u16 x, u16 y, u16 w, u16 h)
{
	...
	if (OMAP_DSS_UPDATE_DISABLED != rfbi.update_mode) {
		if (w == 0 || h == 0)
			return 0;
...
static int rfbi_display_enable(struct omap_dss_device *dssdev)
{
	...
	r = dispc_calc_clock_div(1, DMA_TRANSFER_FREQUENCY, &cinfo);
	if (r) {
		DSSERR("Failed to calc basic clocks\n");
		goto err2;
	}
	...
	REG_FLD_MOD(RFBI_CONTROL, 0, 1, 1); /* clear bypass */
    ...
    /* select CSx */
	REG_FLD_MOD(RFBI_CONTROL, dssdev->phy.rfbi.channel + 1, 3, 2);
