...
static int __init omap_mmc_probe(struct platform_device *pdev)
{
	...
	mmc->f_min	= 400000;		//Card initialization frequency
	mmc->f_max	= 52000000;		//Data transfer frequency
	...
	if (host->id == OMAP_MMC1_DEVID) {
		...
		omap_ctrl_writel(omap_ctrl_readl(OMAPW3G_CONTROL_PBIAS_LITE) &
				(~(1 << 2)), OMAPW3G_CONTROL_PBIAS_LITE);
#else
