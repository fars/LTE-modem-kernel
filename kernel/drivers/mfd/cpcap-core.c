...
struct platform_device cpcap_bt_led = {
	.name		= CPCAP_BT_DEV,
	.id         = -1,
	.dev		= {
		.platform_data  = NULL,
...
static struct platform_device cpcap_usb_adptr_device = {
...
/* FIXME: disable lm3554 until CPCAP<->board
config issue is resolved (IKMAP-3373). */
	&cpcap_lm3554,
#endif
#ifdef CONFIG_CPCAP_USB
	&cpcap_usb_device,
...
/* use to read write FLASH_MODE bit */
inline int cpcap_backup_memory_read(unsigned short *value_ptr)
{
    return cpcap_regacc_read(misc_cpcap, CPCAP_REG_VAL1, value_ptr);
}

inline void cpcap_backup_memory_write(unsigned short value, unsigned short mask)
{
    cpcap_regacc_write(misc_cpcap, CPCAP_REG_VAL1, value, mask);
}

inline void cpcap_vbusswitch_set(unsigned short enable)
{
    if (enable)
        cpcap_regacc_write(misc_cpcap, CPCAP_REG_VUSBC, 0x1, 0x1);
    else
        cpcap_regacc_write(misc_cpcap, CPCAP_REG_VUSBC, 0x0, 0x1);
}
